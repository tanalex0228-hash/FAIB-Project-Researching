...
METABASE_BIND_IP="${METABASE_BIND_IP:-${TAILSCALE_IP}}"
METABASE_PORT="${METABASE_PORT:-3000}"
VOLUME_NAME="${VOLUME_NAME:-fred_macro_metabase_data}"
RECREATE_METABASE="${RECREATE_METABASE:-0}"

docker network create "${NETWORK_NAME}" >/dev/null 2>&1 || true
docker network connect "${NETWORK_NAME}" "${POSTGRES_CONTAINER}" >/dev/null 2>&1 || true

if [[ "${RECREATE_METABASE}" == "1" ]] && docker ps -a --format '{{.Names}}' | grep -qx "${METABASE_CONTAINER}"; then
  docker rm -f "${METABASE_CONTAINER}" >/dev/null
fi
...
if docker ps -a --format '{{.Names}}' | grep -qx "${METABASE_CONTAINER}"; then
  ...
  docker run -d \
    ...
    -p "${METABASE_BIND_IP}:${METABASE_PORT}:3000" \
